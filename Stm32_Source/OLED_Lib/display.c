...
void WriteDisplayNoClear(char *data , FontDef_t* Font, DisplayPixel_t color , uint8_t x , uint8_t y)
{
		SSD1306_GotoXY (x , y);
	  SSD1306_Puts(data , Font, color == DISPLAY_BLUE_COLOR ? SSD1306_COLOR_WHITE : SSD1306_COLOR_BLACK);
    SSD1306_UpdateScreen();
}
...
void DrawPage(PageContainer_t *container)
{
		...
		if(container->pageIndex == 0) {
			WriteDisplayNoClear(container->header , &Font_7x10 , DISPLAY_BLUE_COLOR , 40 , 5);
			WriteDisplayNoClear(container->line_one , &Font_7x10 , DISPLAY_BLUE_COLOR , 15 , 20);
			WriteDisplayNoClear(container->line_two , &Font_7x10 , DISPLAY_BLUE_COLOR , 15 , 35);
			WriteDisplayNoClear(container->line_three , &Font_7x10 , DISPLAY_BLUE_COLOR , 15 , 50);
		}
		else {
			WriteDisplayNoClear(container->header , &Font_7x10 , DISPLAY_BLUE_COLOR , 5 , 5);
			WriteDisplayNoClear(container->line_one , &Font_7x10 , DISPLAY_BLUE_COLOR , 5 , 20);
			WriteDisplayNoClear(container->line_two , &Font_7x10 , DISPLAY_BLUE_COLOR , 5 , 35);
			WriteDisplayNoClear(container->line_three , &Font_7x10 , DISPLAY_BLUE_COLOR , 5 , 50);
		}
}
